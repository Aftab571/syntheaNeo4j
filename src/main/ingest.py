...
class LocalServer(object):
    ...
    def load_csv(self, file):
        with self._driver.session(**self.db_config) as session:
            ...
            row_chunks = pd.read_csv(openfile, dtype=str, encoding='ascii', sep=params['field_sep'], error_bad_lines=False,
                                     index_col=False, skiprows=params['skip_records'], names=header,
                                     low_memory=False, engine='c', compression='infer', header=None,
                                     chunksize=params['chunk_size'])
            #or 'FIRST' in row_chunks or 'LAST' in row_chunks or 'MAIDEN' in row_chunks or 'SUFFIX' in row_chunks:
            #print(row_chunks)
            #if 'NAME' in row_chunks:
             #   row_chunks['NAME'] = row_chunks['NAME'].apply(remove_non_ascii) 
            ...
            for i, rows in enumerate(row_chunks):
                ...
                if 'NAME' in rows:
                    for index, value in rows['NAME'].items():
                        rows['NAME'][index] = self.remove_non_ascii(value)
                if 'FIRST' in rows:
                    for index, value in rows['FIRST'].items():
                        rows['FIRST'][index] = self.remove_non_ascii(value)
                if 'LAST' in rows:
                    for index, value in rows['LAST'].items():
                        rows['LAST'][index] = self.remove_non_ascii(value)
                if 'MAIDEN' in rows:
                    for index, value in rows['MAIDEN'].items():
                        rows['MAIDEN'][index] = self.remove_non_ascii(value)
                if 'SUFFIX' in rows:
                    for index, value in rows['SUFFIX'].items():
                        rows['SUFFIX'][index] = self.remove_non_ascii(value)
                if 'RACE' in rows:
                    for index, value in rows['RACE'].items():
                        rows['RACE'][index] = self.remove_non_ascii(value)
                        
                if 'ETHNICITY' in rows:
                    for index, value in rows['ETHNICITY'].items():
                        rows['ETHNICITY'][index] = self.remove_non_ascii(value)
                if 'BIRTHPLACE' in rows:
                    for index, value in rows['BIRTHPLACE'].items():
                        rows['BIRTHPLACE'][index] = self.remove_non_ascii(value)
                if 'ADDRESS' in rows:
                    for index, value in rows['ADDRESS'].items():
                        rows['ADDRESS'][index] = self.remove_non_ascii(value)
                if 'CITY' in rows:
                    for index, value in rows['CITY'].items():
                        rows['CITY'][index] = self.remove_non_ascii(value)
                if 'MARITAL' in rows:
                    for index, value in rows['MARITAL'].items():
                        rows['MARITAL'][index] = self.remove_non_ascii(value)
                if 'GENDER' in rows:
                    for index, value in rows['GENDER'].items():
                        rows['GENDER'][index] = self.remove_non_ascii(value)
                if 'STATE' in rows:
                    for index, value in rows['STATE'].items():
                        rows['STATE'][index] = self.remove_non_ascii(value)
                if 'COUNTY' in rows:
                    for index, value in rows['COUNTY'].items():
                        rows['COUNTY'][index] = self.remove_non_ascii(value)
                if 'PREFIX' in rows:
                    for index, value in rows['PREFIX'].items():
                        rows['PREFIX'][index] = self.remove_non_ascii(value)
...
def main():
    configuration = 'config.yml'#sys.argv[1]
